...
function PruebaLLamado()
{ 
 //  alert("Llega  Prueba llamado"+  1);
}
...
function EnviarValores() {
  ...
  //  alert("Llega  optener valores libro action ");
    ...
    ajax.open("GET", "./negocioaction.do?method=guardarNegocio" + "&nombre=" + nombre + "&ubicacion=" + ubicacion+"&precioEntrada="+precioEntrada+"&descripcion="+descripcion+"&distrito="+Distrito+ "&texto=" + text, true);
...
$(document).ready(function() {
    ...
    var valorComboDistritos = $("#selDistritoBusqueda").val();
  //  alert(valorComboDistritos);
   ...
   function operacion(){
      // alert("LLEGA A OPERACION   :) ");
       obtenerDistritoBusqueda;
       
       var capa = document.getElementById('tabla');
       var nombre_Negocio= document.getElementById("nombre");
       
      //  alert ("Distrito  y nombre    " + Distrito + nombre_Negocio );
       var ajax = new nuevoAjax();
       ajax.open("GET","./negocioaction.do?method=obtenerNegocioParaInsertarImagen &Distrito="+Distrito+"&Nombre="+ nombre_Negocio,true);//lo que refresca
       ajax.onreadystatechange=function(){
           if(ajax.readyState==4){
               capa.innerHTML=ajax.responseText;
           }
